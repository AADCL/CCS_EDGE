...
配套 CCS 0.23.1：[完整使用手册](../documents/USER_MANUAL.md#documents-user-manual-md) · [设备内接口与参数](../documents/INTERFACE_REFERENCE.md#documents-interface-reference-md)。多数包级 launch 默认读取共享配置包；epgeneral_mqtav 0.5.0 和 epgeneral_udp_telemetry 0.4.0、epgeneral_video_srt 0.2.0 须显式选择 config_dir；一键脚本显式读取工作空间 `config/<profile>`，修改后需重启。

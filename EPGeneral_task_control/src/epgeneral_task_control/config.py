        ...
        if adapter_type == "navigation":
            ...
            if adapter.get("auto_arm_on_schedule", False) or adapter.get("auto_disarm_on_terminal", False):
                ...
                for key in ("control_diagnostics_topic", "control_diagnostics_status", "control_diagnostics_key"):
                    if key in adapter:
                        _text(adapter, key, "adapter." + key)
                for key in ("control_stop_service", "control_heartbeat_topic"):
                    if key in adapter:
                        _text(adapter, key, "adapter." + key)
                if "clear_costmaps_service" in adapter:
                    _text(adapter, "clear_costmaps_service",
                          "adapter.clear_costmaps_service")
                    _number(adapter.get("clear_costmaps_settle_seconds", 0.30),
                            "adapter.clear_costmaps_settle_seconds", 0.01, 5.0)
                elif "clear_costmaps_settle_seconds" in adapter:
                    raise ConfigError(
                        "adapter.clear_costmaps_settle_seconds requires "
                        "adapter.clear_costmaps_service")
                if "control_heartbeat_hz" in adapter:
                    _number(adapter.get("control_heartbeat_hz"),
                            "adapter.control_heartbeat_hz", 1.0, 100.0)
                if not isinstance(adapter.get("reset_control_on_emergency_clear", False), bool):
                    raise ConfigError(
            ...
            if authority is not None:
                if not isinstance(authority, dict):
                    raise ConfigError("adapter.control_authority must be a mapping")
                for key in (
                ...
                _number(authority.get("odom_timeout_seconds", 1.0),
                        "adapter.control_authority.odom_timeout_seconds", 0.1, 10.0)

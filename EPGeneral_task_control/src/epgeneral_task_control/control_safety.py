...
class NavigationControlSafety(object):
    ...
    def start(self, reset_callback):
        ...
        if self.config.get("clear_costmaps_service"):
            self.clear_costmaps_client = self.rospy.ServiceProxy(
                self.config["clear_costmaps_service"], Empty)
        if self.config.get("control_stop_service"):
            self.stop_client = self.rospy.ServiceProxy(
                self.config["control_stop_service"], Trigger)
        if self.config.get("control_heartbeat_topic"):
            from std_msgs.msg import Empty
            self.heartbeat_class = Empty
            self.heartbeat_pub = self.rospy.Publisher(
                self.config["control_heartbeat_topic"], Empty, queue_size=1)
            period = 1.0 / float(self.config.get("control_heartbeat_hz", 10.0))
            self.heartbeat_timer = self.rospy.Timer(
                self.rospy.Duration(period), self._heartbeat_callback)
    ...
    def _heartbeat_callback(self, unused_event=None):
        with self.lock:
            enabled = self.heartbeat_enabled
        if enabled and self.heartbeat_pub is not None:
            self.heartbeat_pub.publish(self.heartbeat_class())
    ...
    def clear_latch(self):
        ...
        try:
            ...
            try:
                if self.config.get("reset_control_on_emergency_clear", False):
                    response = self.reset_client()
                    self._require_success(response, "control reset")
    ...
    def _clear_costmaps_before_enable(self):
        service_name = self.config.get("clear_costmaps_service")
        if not service_name:
            return
        self._rpc(service_name, self.clear_costmaps_client)
        deadline = time.monotonic() + float(
            self.config.get("clear_costmaps_settle_seconds", 0.30))
        while True:
            self._cancel_check()
            self.assert_localized()
        ...
        if not self._control_matches(False):
            raise ControlSafetyError(
                "fresh disabled state was lost while clearing costmaps",
                "CONTROL_STATE_UNAVAILABLE")
    ...
    def arm(self):
        ...
        try:
            ...
            self._clear_costmaps_before_enable()
            self._cancel_check()
            self.assert_localized()
            since = time.monotonic()
            with self.lock:
    ...
    def stop(self, reason):
        ...
        if self.stop_client is not None:
            try:
                response = self._rpc(
                    self.config["control_stop_service"], self.stop_client)
                self._require_success(response, "control stop")
            except Exception as exc:
        ...
        if errors:
            raise ControlSafetyError(
                "; ".join(errors), "CONTROL_STOP_FAILED")

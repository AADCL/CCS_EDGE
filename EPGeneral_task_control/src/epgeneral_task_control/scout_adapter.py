...
def move_base_failure(state, status_text, waypoint_index):
    ...
    if state == 4 and ("valid plan" in text.lower() or "plan" in text.lower()):
        ...
        code = "NAVIGATION_ACTION_LOST"
    else:
        code = "NAVIGATION_ACTION_FAILED"
    return ScoutAdapterError(detail, code)


def waypoint_timeout_failure(waypoint_index):
    return ScoutAdapterError(
        "waypoint %d timed out" % waypoint_index, "NAVIGATION_WAYPOINT_TIMEOUT")


_RECOVERABLE_NAVIGATION_FAILURES = frozenset((
    "NAVIGATION_ACTION_FAILED",
    "NAVIGATION_GOAL_PREEMPTED",
...
def terminal_failure_requires_fault_stop(state, error_code):
    """Return whether a terminal result must engage the persistent safety stop."""
    if state == "emergency_stopped":
        return True
    return state == "failed" and error_code not in _RECOVERABLE_NAVIGATION_FAILURES
...
class ScoutNavigationAdapter(object):
    ...
    def _launch_navigation(self, map_id):
        ...
        command = ["roslaunch", self.config["navigation_launch_package"],
                   ...
                   "map_dir:=" + map_dir, "nav_map_yaml:=" + map_yaml,
                    "odom_topic:=" + self.config.get(
                        "navigation_odom_topic", self.config["odom_topic"]),
                   "cmd_vel_topic:=" + self.config.get(
                       "navigation_cmd_vel_topic", self.config["zero_velocity_topic"])]
        self.rospy.loginfo("starting Scout navigation: %s", " ".join(command))
        stdout = subprocess.DEVNULL
        log_directory = self.config.get("navigation_log_directory")
        if log_directory:
            directory = os.path.abspath(os.path.expanduser(log_directory))
            os.makedirs(directory, exist_ok=True)
            self.navigation_log_path = os.path.join(
                directory, "navigation-%s.log" % map_id)
            self.navigation_log = open(self.navigation_log_path, "a", buffering=1)
            self.navigation_log.write("\n=== navigation start %.3f ===\n" % time.time())
            self.navigation_log.write("command: %s\n" % " ".join(command))
            stdout = self.navigation_log
        return self.process_factory(command, stdout=stdout, stderr=subprocess.STDOUT,
                                    universal_newlines=True, preexec_fn=os.setsid)

    def _navigation_exit_message(self, startup=False):
        process = self.navigation_process
        code = process.poll() if process is not None else None
        prefix = (
            "navigation process exited during startup"
            if startup else "navigation process exited")
        detail = "%s (exit code %s)" % (
            prefix, code if code is not None else "unknown")
        if self.navigation_log is not None:
            self.navigation_log.flush()
        if self.navigation_log_path:
            try:
                with open(self.navigation_log_path, "r") as stream:
                    lines = [
                        line.strip() for line in stream.readlines()[-40:]
                        if line.strip()]
                if lines:
                    detail += ": " + lines[-1][:512]
            except (IOError, OSError):
                pass
            detail += "; log=" + self.navigation_log_path
        return detail
            ...
            for index, (point, yaw) in enumerate(zip(payload["waypoints"], yaws)):
                ...
                while not client.wait_for_result(self.rospy.Duration(0.2)):
                    if self.stop_event.is_set():
                        return
                    if self._process_exited():
                        raise ScoutAdapterError(self._navigation_exit_message())
                    if self.control_armed:
                        self.control_safety.assert_running()
                    if time.monotonic() >= waypoint_deadline:
                        client.cancel_goal()
                        raise waypoint_timeout_failure(index)
    ...
    def _publish_zero(self):
        # Service-backed platforms stop through their authority coordinator.
        # Publishing directly would bypass caller validation in the safety gate.
        if self.zero_pub is None or self.control_safety is not None:
            return
        from geometry_msgs.msg import Twist
    ...
    def watchdog(self, unused_event=None):
        ...
        if self.control_safety is not None and self.navigation_process is not None and self._process_exited():
            ...
            if record is not None:
                self._finish(record["command"], "failed", -1, 0.0,
                             self._navigation_exit_message(), "NAVIGATION_PROCESS_EXITED", force=True,
        ...
        with self.lock:
            ...
            execution = self.execution
            prepared = self.prepared
            exit_message = self._navigation_exit_message()
            self.navigation_process = None
            self.navigation_map_id = None
            self.execution = None
            self.prepared = None
        ...
        if execution is not None:
            self._feedback(execution["command"], "failed", execution["waypoint_index"], 0.0,
                           exit_message, "NAVIGATION_PROCESS_EXITED")

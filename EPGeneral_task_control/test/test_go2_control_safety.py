...
class ControlSafetyTests(unittest.TestCase):
    ...
    def test_start_creates_optional_empty_costmap_clear_client(self):
        empty, trigger, set_bool = object(), object(), object()
        self.control.config["clear_costmaps_service"] = (
        ...
        with patch.dict(sys.modules, modules):
            self.control.start(Mock())
        self.assertEqual(self.rospy.ServiceProxy.call_args_list[2].args,
                         ("/move_base/clear_costmaps", empty))

    def test_wheeltec_clears_costmap_while_disabled_before_enable(self):
        order = []
        self.control.config.update(
            clear_costmaps_service="/move_base/clear_costmaps",
            clear_costmaps_settle_seconds=0.01)
        self.control.reset_client.side_effect = lambda: (
            order.append("reset") or response())
        self.control.clear_costmaps_client = Mock(
            side_effect=lambda: order.append("clear") or object())

        def enable(value):
            order.append("enable" if value else "disable")
            self.control.control_callback(types.SimpleNamespace(data=value))
            return response()

        self.control.enable_client.side_effect = enable
        self.control.arm()
        self.assertEqual(order, ["reset", "clear", "enable"])

    def test_costmap_clear_failure_never_enables(self):
        self.control.config.update(
            clear_costmaps_service="/move_base/clear_costmaps",
            clear_costmaps_settle_seconds=0.01)
        self.control.clear_costmaps_client = Mock(
            side_effect=RuntimeError("clear unavailable"))
        with self.assertRaisesRegex(ControlSafetyError, "clear unavailable"):
            self.control.arm()
        self.assertEqual(self.calls, [])

    def test_cancel_during_costmap_settle_never_enables(self):
        self.control.config.update(
            clear_costmaps_service="/move_base/clear_costmaps",
            clear_costmaps_settle_seconds=0.10)
        self.control.clear_costmaps_client = Mock(
            side_effect=lambda: self.stop.set() or object())
    ...
    def test_optional_heartbeat_runs_only_while_control_is_armed(self):
        self.control.heartbeat_pub = Mock()
        self.control.heartbeat_class = Mock(return_value=object())
        self.control.arm()
        self.control._heartbeat_callback()
        self.control.heartbeat_pub.publish.assert_called_once()
        self.control.disarm()
        self.control._heartbeat_callback()
        self.control.heartbeat_pub.publish.assert_called_once()

    def test_optional_stop_service_is_called_before_fault_latch(self):
        self.control.config["control_stop_service"] = "/wheeltec_control/stop"
        self.control.stop_client = Mock(return_value=response())
        self.control.stop("driver heartbeat expired")
        self.control.stop_client.assert_called_once_with()
...
class Go2AdapterTests(unittest.TestCase):
    ...
    def test_configuration_rejects_unsafe_or_incomplete_control_options(self):
        ...
        for delta in ({"auto_arm_on_schedule": "true"}, {"auto_disarm_on_terminal": False},
                      ...
                      {"clear_costmaps_settle_seconds": 0.3}, {}):

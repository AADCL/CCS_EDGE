...
- 修复满点数提前发布及旧窗口积压；单调时钟控制 1 Hz 发布，默认 10 cm 体素与最新单窗口队列。
- 按 PCD 头与 XYZ 正文精确裁剪至 500,000 字节；HTTP 预览共享滚动一秒预算，重复 GET、Range 和并发请求共同计费。
- ACK/淘汰释放缓存时保护在读文件；最终地图保存与成果下载保持独立。
- 新增并发限流、文件生命周期、空间裁剪、严格发布节奏测试；详见 [实时预览契约](../documents/REALTIME_PREVIEW.md)。

## v0.14.0 - 2026-09-21

- 成果包支持 PCD + PGM/YAML、PCD + OT 及全部格式，增加 OT 完整树校验与哈希角色。
- 可配置既有占据图生成包的检查/导出命令、源 OT 与会话输出路径；OT 与原始 PCD 同目录。
- 各适配器保留源时间戳，按会话开始前指纹筛除历史占据图；只有 OT 时跳过 PGM/YAML 专属校验。
- 通过请求中的 artifact_formats 保留旧平台三件套；无法满足时明确报不兼容。
...
- 端侧部署与静态闭环的实际结果单独记录于 [AGV 建图部署日志](../documents/devices/ground_air_agv/DEPLOYMENT_GUIDE.md#documents-ground-air-agv-mapping-deployment-log-md)。

...
PROFILE = ROOT / 'devices/ground_air_agv/profiles/ground_air_agv'
CONTROL = ROOT / 'devices/ground_air_agv/EPGeneral_ground_air_control'
...
class GroundAirAgvProfileTests(unittest.TestCase):
    ...
    def test_startup_adds_task_control_after_coordinate_transforms(self):
        ...
        manager = (CONTROL / 'scripts/ground_air_stage_manager_node.py').read_text(encoding="utf-8")
        core = (CONTROL / 'src/epgeneral_ground_air_control/system_stage_core.py').read_text(encoding="utf-8")
    ...
    def test_deployment_guards_overwritten_runtime_and_final_readiness(self):
        ...
        self.assertIn(
            'add devices/ground_air_agv/profiles/ground_air_agv/config/map_stream.yaml '
    ...
    def test_relocalization_profile_and_scoped_override_contract(self):
        ...
        override = (
            PROFILE / 'overrides/car_bringup/launch/relocalization_system.launch'
    ...
    def test_relocalization_deploys_and_checks_mapping_compatibility(self):
        ...
        self.assertIn(
            'add "EPGeneral_map_stream/${relative}" '
        ...
        self.assertIn(
            'add EPGeneral_device_config/config/map_stream.yaml '
            '"${TEMP_ROOT}/fixtures/map_stream.yaml" 644', deploy)
        self.assertIn(
            'CCS_MAP_STREAM_TEST_MAPPING="${TEMP_ROOT}/fixtures/map_stream.yaml" '
            'PYTHONPATH=', deploy)
        self.assertTrue((ROOT / 'EPGeneral_device_config/config/map_stream.yaml').is_file())

...
add devices/ground_air_agv/profiles/ground_air_agv/start_ccs_edge_dev.sh "${WS}/start_ccs_edge_dev.sh" 755
add devices/ground_air_agv/profiles/ground_air_agv/config/relocalization.yaml "${PROFILE_DIR}/relocalization.yaml" 644
add devices/ground_air_agv/profiles/ground_air_agv/overrides/car_bringup/package.xml "${WS}/overrides/car_bringup/package.xml" 644
add devices/ground_air_agv/profiles/ground_air_agv/overrides/car_bringup/launch/relocalization_system.launch "${WS}/overrides/car_bringup/launch/relocalization_system.launch" 644

for package in EPGeneral_relocalization EPGeneral_ground_air_control; do
  package_source="${package}"
  [[ "${package}" == EPGeneral_ground_air_control ]] && package_source="devices/ground_air_agv/${package}"
  while IFS= read -r source; do
    relative="${source#${package_source}/}"
    mode=644
    [[ "${relative}" == scripts/*.py ]] && mode=755
    add "${source}" "${WS}/src/${package}/${relative}" "${mode}"
  done < <(cd "${ROOT}" && find "${package_source}" -type f ! -path '*/__pycache__/*' ! -name '*.pyc' | sort)
done

# Keep the standard test sample separate from the AGV's live profile.
add EPGeneral_device_config/config/map_stream.yaml "${TEMP_ROOT}/fixtures/map_stream.yaml" 644
...
for relative in \
  ...
  mode=644
  [[ "${relative}" == scripts/*.py ]] && mode=755
  add "EPGeneral_map_stream/${relative}" "${WS}/src/EPGeneral_map_stream/${relative}" "${mode}"
done

for document in DEPLOYMENT_GUIDE.md DEPLOYMENT_RECORD.md; do
  add "documents/devices/ground_air_agv/${document}" "${WS}/documents/${document}" 644
done

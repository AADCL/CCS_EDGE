...
add devices/ground_air_agv/profiles/ground_air_agv/ccs-edge-dev.service "${USER_UNIT}" 644
add devices/ground_air_agv/profiles/ground_air_agv/config/map_stream.yaml "${WS}/config/ground_air_agv/map_stream.yaml" 644
for name in manual_mapping_control.launch mapping_coordinate_transforms.launch relocalization_control.launch; do
  add "devices/ground_air_agv/profiles/ground_air_agv/launch/${name}" "${CAR}/launch/${name}" 644
done
for name in ground_air_stage_manager_node.py system_stage_runtime.py system_stage_core.py; do
  add "devices/ground_air_agv/profiles/ground_air_agv/car_bringup_scripts/${name}" "${CAR}/scripts/${name}" 755
done
for name in test_system_stage_core.py test_ground_air_stage_manager.py test_stage_manager_contract.py; do
  add "devices/ground_air_agv/profiles/ground_air_agv/car_bringup_tests/${name}" "${CAR}/tests/${name}" 644
done
for name in CMakeLists.txt src/epgeneral_map_stream/config.py test/test_ground_air_backend.py test/test_ground_air_stage_client.py; do
  add "EPGeneral_map_stream/${name}" "${WS}/src/EPGeneral_map_stream/${name}" 644
done
for name in ground_air_mapping_stack.sh ground_air_stage_client.py; do
  add "EPGeneral_map_stream/scripts/${name}" "${WS}/src/EPGeneral_map_stream/scripts/${name}" 755
done
for name in DEPLOYMENT_GUIDE.md DEPLOYMENT_RECORD.md; do
  add "documents/devices/ground_air_agv/${name}" "${WS}/documents/${name}" 644
done

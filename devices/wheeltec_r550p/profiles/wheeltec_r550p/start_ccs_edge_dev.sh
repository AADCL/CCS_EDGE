...
NATIVE_WORKSPACE="${CCS_LIVOX_SETUP:-/home/nrc19/livox_fastlio/devel/setup.bash}"
...
owned_process_alive() {
  local info parent group session status
  [[ "$1" =~ ^[0-9]+$ ]] || return 1
...
process_alive() {
  local status
  status="$(ps -o stat= -p "$1" 2>/dev/null)" || return 1
  [[ -n "${status}" && "${status// /}" != Z* ]]
}
stop_process() {
  local pid="$1" attempt
  process_alive "${pid}" || { wait "${pid}" 2>/dev/null || true; return 0; }
...
release_control() {
  # V6 uses a protected velocity channel and explicit manual-authority release.
  run_quiet timeout 8 rosservice call /wheeltec_robot/set_autonomous 'data: false' || return 1
  run_quiet timeout 10 rostopic pub -1 /nav_cmd_vel geometry_msgs/Twist \
    '{linear: {x: 0.0, y: 0.0, z: 0.0}, angular: {x: 0.0, y: 0.0, z: 0.0}}'
}
start_optional_video() {
  local output
  VIDEO_OWNED=true
  if output="$(timeout -k 5 100 "${WORKSPACE}/manage_ccs_video.sh" start 8>&- 9>&- 2>&1)"; then
    report OK "${output}"
    [[ "${output}" == *'STARTED '* ]] || VIDEO_OWNED=false
  else
    report WARN "Video degraded; base services continue: ${output}"
    if run_quiet timeout -k 5 55 "${WORKSPACE}/manage_ccs_video.sh" stop; then VIDEO_OWNED=false; fi
  fi
}
shutdown_all() {
  ...
  if [[ -n "${PIDS[0]}" ]] && owned_process_alive "${PIDS[0]}" && master_exists; then
    release_control || { report ERROR "Manual release/zero velocity failed."; exit_code=1; }
    run_quiet python3 "${READINESS}" stopped --timeout 10 || { report ERROR "Fresh zero wheel speed not confirmed."; exit_code=1; }
  fi
  if [[ "${VIDEO_OWNED}" == true ]]; then
    run_quiet timeout -k 5 55 "${WORKSPACE}/manage_ccs_video.sh" stop || exit_code=1
  fi
...
acquire_startup_locks() {
  mkdir -p "${STATE_DIR}"
  # Also lock the former entry's file so an already-running old deployment is rejected.
  exec 8>"${WORKSPACE}/run/startup.lock"
  flock -n 8 || fail "An existing UGV_003 workflow owns the legacy startup lock."
  exec 9>"${STATE_DIR}/startup.lock"
  flock -n 9 || fail "The UGV_003 workflow is already running."
  }

# Sourcing defines the same lifecycle functions for regression tests only.
...
export CCS_EDGE_WORKSPACE="${WORKSPACE}" CCS_LIVOX_SETUP="${NATIVE_WORKSPACE}/devel/setup.bash"
export CCS_EDGE_PROFILE_CONFIG_DIR="${PROFILE_CONFIG_DIR}" CCS_EDGE_STATE_DIR="${WORKSPACE}/run"
export CCS_ALGORITHM_LOCK_FILE="${WORKSPACE}/run/algorithm.lock"
export ROS_HOME="${WORKSPACE}/run/ros_home" ROS_LOG_DIR="${WORKSPACE}/log/ros"
...
if [[ "${CCS_ENABLE_VIDEO:-1}" != 0 ]]; then
  gst-inspect-1.0 srtsink >/dev/null 2>&1 && gst-inspect-1.0 x264enc >/dev/null 2>&1 \
    || report WARN "Video dependencies are unavailable; base services may still start."
fi
...
launch() {
  ...
  setsid roslaunch "$@" >"${LOG_DIR}/${name}.log" 2>&1 </dev/null 8>&- 9>&- &
...
if [[ "${CHECK_ONLY}" != true ]]; then
  if master_exists; then
    ...
    for node in "${NODE_NAMES[@]}" /livox_lidar_publisher2 /epgeneral_navigation_task_adapter \
      /wheeltec_control /laserMapping /wheeltec_pointcloud_mapper /wheeltec_tf_manager /wheeltec_pose_adapter \
  ...
  export ROS_LOG_DIR="${LOG_DIR}/ros" CCS_EDGE_VIDEO_LOG_DIR="${LOG_DIR}/video"
  export CCS_OWNER_TOKEN="${RUN_ID}"
  mkdir -p "${WORKSPACE}/run/state"
  printf '%s\n' '{"schema_version":2,"status":"idle","map_id":""}' >"${WORKSPACE}/run/state/relocalization.json.tmp"
  mv -f "${WORKSPACE}/run/state/relocalization.json.tmp" "${WORKSPACE}/run/state/relocalization.json"
  printf '%s\n' "$$" >"${STATE_DIR}/startup.pid"
  trap 'shutdown_all 130' INT
  trap 'shutdown_all 143' TERM
  trap 'shutdown_all $?' EXIT
  record "startup pid=$$ UGV_003 video=${CCS_ENABLE_VIDEO:-1}"
  record "time synchronization: ${time_sync_output}"
  if ! master_exists; then
    setsid roscore >"${LOG_DIR}/roscore.log" 2>&1 </dev/null 8>&- 9>&- &
...
launch 0 epgeneral_wheeltec_integration base.launch
if [[ "${CHECK_ONLY}" != true ]]; then
  run_quiet python3 "${READINESS}" inputs --timeout 30 || fail "Base sensor inputs are not fresh."
  release_control || fail "Initial manual release/zero velocity failed."
...
launch 5 epgeneral_task_control wheeltec_task_control.launch task_config_file:="${PROFILE_CONFIG_DIR}/task_control.yaml" \
  device_config_file:="${PROFILE_CONFIG_DIR}/device.yaml"
if [[ "${CHECK_ONLY}" == true ]]; then
  report OK "UGV_003 configuration, dependencies and launches passed; no nodes were started."
  exit 0
fi
ros_node_exists /epgeneral_navigation_task_adapter || fail "Navigation task adapter is absent."
ros_node_exists /wheeltec_control || fail "Wheeltec authority coordinator is absent."
for service in /wheeltec_robot/set_autonomous /wheeltec_control/enable; do
  [[ "$(timeout 5 rosservice type "${service}")" == std_srvs/SetBool ]] || fail "Required service unavailable: ${service}"
done
if [[ "${CCS_ENABLE_VIDEO:-1}" != 0 ]]; then start_optional_video; fi
report READY "UGV_003 standby; mapping/localization/navigation remain task-managed. Logs: ${LOG_DIR}"
while true; do
  sleep 2
  ...
  for attempt in 1 2 3; do
    ...
    if nodes="$(timeout 5 rosnode list 2>>"${LOG_DIR}/runtime_monitor.log")"; then
      missing=false
      for node in "${NODE_NAMES[@]}" /livox_lidar_publisher2 /epgeneral_navigation_task_adapter /wheeltec_control; do

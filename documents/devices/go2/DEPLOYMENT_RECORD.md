...
<a id="deploy-records-qrd-001-deployment-md"></a>

<a id="deploy-records-qrd-001-deployment-md-qrd_001-部署与验收记录"></a>
# QRD_001 部署与验收记录

合并日期：2026-09-11；操作基线：CCS 0.23.1。此文件是该设备唯一的部署记录入口，后续按日期追加。

<a id="deploy-records-qrd-001-deployment-md-当前入口"></a>
## 当前入口

- 设备：QRD_001；profile：`go2_edu`；端侧：`nvidia@192.168.50.100`。
- [配置与脚本](../../../devices/go2/profiles/go2_edu)保持原位置；[从零部署](../../USER_MANUAL.md#documents-deployment-guide-md)、[接口填写](../../INTERFACE_REFERENCE.md#documents-config-topic-reference-md)、[使用手册](../../USER_MANUAL.md#documents-user-manual-md)、[设备索引](../../../README.md)。
- legacy：重定位 disabled，根脚本不启动任务。不能套用原生 Robot2/Robot3 服务与日志路径。

<a id="deploy-records-qrd-001-deployment-md-历史材料与来源校验"></a>
## 历史材料与来源校验
...
| --- | --- | --- |
| `deploy/go2_edu/DEPLOYMENT.md` | `f4939e9edd00cad234a6693439feaea58fe177762362fa6366507ee66a096f17` | [材料 1](DEPLOYMENT_RECORD.md#deploy-records-qrd-001-deployment-md-source-1) |

<a id="deploy-records-qrd-001-deployment-md-source-1"></a>

<a id="deploy-records-qrd-001-deployment-md-历史材料-1deploygo2_edudeploymentmd"></a>
## 历史材料 1：deploy/go2_edu/DEPLOYMENT.md

> 归档原文；以下命令、状态与结论按原日期理解。

<a id="deploy-records-qrd-001-deployment-md-s1-go2-edu-端侧部署"></a>

<a id="deploy-records-qrd-001-deployment-md-go2-edu-端侧部署"></a>
### Go2 EDU 端侧部署
...
<a id="deploy-records-qrd-001-deployment-md-s1-发布内容"></a>

<a id="deploy-records-qrd-001-deployment-md-发布内容"></a>
#### 发布内容
...
<a id="deploy-records-qrd-001-deployment-md-s1-数据源"></a>

<a id="deploy-records-qrd-001-deployment-md-数据源"></a>
#### 数据源
...
<a id="deploy-records-qrd-001-deployment-md-s1-安装"></a>

<a id="deploy-records-qrd-001-deployment-md-安装"></a>
#### 安装
...
<a id="deploy-records-qrd-001-deployment-md-s1-验证与回滚"></a>

<a id="deploy-records-qrd-001-deployment-md-验证与回滚"></a>
#### 验证与回滚
...
<a id="deploy-records-qrd-001-deployment-md-后续记录填写格式"></a>
## 后续记录填写格式

追加日期/范围、源码版本与差异、文件清单及前后哈希、备份/证据路径、命令与实测、告警/未测项、启停/回滚。回滚不覆盖更新的急停状态，文档整理不表示重新部署。

<a id="deploy-records-qrd-002-deployment-md"></a>

<a id="deploy-records-qrd-002-deployment-md-qrd_002-部署与验收记录"></a>
# QRD_002 部署与验收记录

合并日期：2026-09-11；操作基线：CCS 0.23.1。此文件是该设备唯一的部署记录入口，后续按日期追加。

<a id="deploy-records-qrd-002-deployment-md-当前入口"></a>
## 当前入口

- 设备：QRD_002；profile：`go2_robot2`；端侧：`unitree@192.168.50.111`。
- [配置与脚本](../../../devices/go2/profiles/go2_robot2)保持原位置；[从零部署](../../USER_MANUAL.md#documents-deployment-guide-md)、[接口填写](../../INTERFACE_REFERENCE.md#documents-config-topic-reference-md)、[使用手册](../../USER_MANUAL.md#documents-user-manual-md)、[设备索引](../../../README.md)。
- 原生 Go2，重定位管理导航，任务 attach。2026-09-12 已部署退出急停修复、服务复位及受控重启，联合下发通过，最终 CCS 运行且 disabled；定位 standby，未做运动验收。见[最新记录](DEPLOYMENT_RECORD.md#deploy-records-qrd-002-deployment-md-joint-shutdown-20260912)。Robot2 仍未配置独立 MQTT connection，授时/日志沿用本设备策略，不能视为同步了 Robot3 全部行为。旧部署请求仅为历史需求，不是新的授权或验收事实。

- 2026-09-12 后续：用户已确认本轮落地测试完成，见[现场确认](DEPLOYMENT_RECORD.md#deploy-records-qrd-002-deployment-md-field-confirmation-20260912)。此前 disabled/standby 是工具验收时快照，不作为当前实时状态；后续未部署修订的状态保持。

<a id="deploy-records-qrd-002-deployment-md-历史材料与来源校验"></a>
## 历史材料与来源校验
...
| 原文件（相对 edge_side_pkg） | 原始字节 SHA-256 | 合并章节 |
| --- | --- | --- |
| `deploy/go2_robot2/README.md` | `c92cf7c18ffa7dc70068f2908e53c9fe9cb5c45664f3fba8454e2dda905bb0c9` | [材料 1](DEPLOYMENT_RECORD.md#deploy-records-qrd-002-deployment-md-source-1) |
| `deploy/go2_robot2/DEPLOYMENT.md` | `905404f16f038983f8028b35a868f0f9752ba2cf78f600e8b6a2c4514406f626` | [材料 2](DEPLOYMENT_RECORD.md#deploy-records-qrd-002-deployment-md-source-2) |
| `deploy/go2_robot2/VALIDATION.md` | `79e5846569096599ab810c9548caf1e598202f08559d73626e052ea7fa6fcba7` | [材料 3](DEPLOYMENT_RECORD.md#deploy-records-qrd-002-deployment-md-source-3) |
| `deploy/EDGE_DEVICE_DEPLOYMENT_REQUEST.md` | `a220214acc20c74f0bf6815219934b600bd6122d0035b698cdc109ce1df46ca5` | [材料 4](DEPLOYMENT_RECORD.md#deploy-records-qrd-002-deployment-md-source-4) |

<a id="deploy-records-qrd-002-deployment-md-source-1"></a>

<a id="deploy-records-qrd-002-deployment-md-历史材料-1deploygo2_robot2readmemd"></a>
## 历史材料 1：deploy/go2_robot2/README.md

> 归档原文；以下命令、状态与结论按原日期理解。

<a id="deploy-records-qrd-002-deployment-md-s1-go2-robot-2-端侧部署"></a>

<a id="deploy-records-qrd-002-deployment-md-go2-robot-2-端侧部署"></a>
### Go2 Robot 2 端侧部署
...
- [部署结构、日常启动与原生 launch 参数](DEPLOYMENT_GUIDE.md#deploy-go2-robot2-deployment-md)
- [本次真实部署结果、增量测试、已知状态与备份校验值](DEPLOYMENT_GUIDE.md#deploy-go2-robot2-validation-md)
- [一次性迁移、最终清理与回滚用法](DEPLOYMENT_GUIDE.md#deploy-go2-robot2-validation-md-%E8%BF%81%E7%A7%BB%E4%B8%8E%E5%9B%9E%E6%BB%9A%E7%94%A8%E6%B3%95)
- [迁移脚本](../../../devices/go2/profiles/go2_robot2/migrate_workspace.sh)

当前物理目录迁移与 `vendor/bin` 清理已经完成；日常只需使用工作空间根
`start_ccs_edge_dev.sh`，不要重复执行一次性迁移。真实运动尚未验收。

<a id="deploy-records-qrd-002-deployment-md-source-2"></a>

<a id="deploy-records-qrd-002-deployment-md-历史材料-2deploygo2_robot2deploymentmd"></a>
## 历史材料 2：deploy/go2_robot2/DEPLOYMENT.md

> 归档原文；以下命令、状态与结论按原日期理解。

<a id="deploy-records-qrd-002-deployment-md-s2-go2-robot-2-deployment"></a>

<a id="deploy-records-qrd-002-deployment-md-go2-robot-2-deployment"></a>
...
See [the deployment validation record](DEPLOYMENT_GUIDE.md#deploy-go2-robot2-validation-md) for measured results,
backup checksums, known historical state and validation limits. The
[migration and rollback procedure](DEPLOYMENT_GUIDE.md#deploy-go2-robot2-validation-md-%E8%BF%81%E7%A7%BB%E4%B8%8E%E5%9B%9E%E6%BB%9A%E7%94%A8%E6%B3%95) gives the exact
one-time command workflow for [migrate_workspace.sh](../../../devices/go2/profiles/go2_robot2/migrate_workspace.sh).

<a id="deploy-records-qrd-002-deployment-md-s2-source-of-truth-and-layout"></a>

<a id="deploy-records-qrd-002-deployment-md-source-of-truth-and-layout"></a>
...
<a id="deploy-records-qrd-002-deployment-md-s2-startup-and-native-parameters"></a>

<a id="deploy-records-qrd-002-deployment-md-startup-and-native-parameters"></a>
...
<a id="deploy-records-qrd-002-deployment-md-s2-failure-analysis-and-compatibility"></a>

<a id="deploy-records-qrd-002-deployment-md-failure-analysis-and-compatibility"></a>
...
<a id="deploy-records-qrd-002-deployment-md-s2-migration-and-clock-service"></a>

<a id="deploy-records-qrd-002-deployment-md-migration-and-clock-service"></a>
...
<a id="deploy-records-qrd-002-deployment-md-s2-incremental-acceptance"></a>

<a id="deploy-records-qrd-002-deployment-md-incremental-acceptance"></a>
...
The completed 2026-09-09 rollout is recorded in [VALIDATION.md](DEPLOYMENT_GUIDE.md#deploy-go2-robot2-validation-md).
...
<a id="deploy-records-qrd-002-deployment-md-source-3"></a>

<a id="deploy-records-qrd-002-deployment-md-历史材料-3deploygo2_robot2validationmd"></a>
## 历史材料 3：deploy/go2_robot2/VALIDATION.md

> 归档原文；以下命令、状态与结论按原日期理解。

<a id="deploy-records-qrd-002-deployment-md-s3-go2-robot-2-部署与增量验证记录"></a>

<a id="deploy-records-qrd-002-deployment-md-go2-robot-2-部署与增量验证记录"></a>
### Go2 Robot 2 部署与增量验证记录

<a id="deploy-records-qrd-002-deployment-md-s3-验证范围与版本"></a>

<a id="deploy-records-qrd-002-deployment-md-验证范围与版本"></a>
#### 验证范围与版本
...
<a id="deploy-records-qrd-002-deployment-md-s3-已完成结果"></a>

<a id="deploy-records-qrd-002-deployment-md-已完成结果"></a>
#### 已完成结果
...
<a id="deploy-records-qrd-002-deployment-md-s3-udp-真机接收"></a>

<a id="deploy-records-qrd-002-deployment-md-udp-真机接收"></a>
##### UDP 真机接收
...
<a id="deploy-records-qrd-002-deployment-md-s3-控制状态与安全边界"></a>

<a id="deploy-records-qrd-002-deployment-md-控制状态与安全边界"></a>
##### 控制状态与安全边界
...
<a id="deploy-records-qrd-002-deployment-md-s3-旧状态说明"></a>

<a id="deploy-records-qrd-002-deployment-md-旧状态说明"></a>
#### 旧状态说明
...
<a id="deploy-records-qrd-002-deployment-md-s3-增量测试"></a>

<a id="deploy-records-qrd-002-deployment-md-增量测试"></a>
#### 增量测试
...
<a id="deploy-records-qrd-002-deployment-md-s3-备份与载荷"></a>

<a id="deploy-records-qrd-002-deployment-md-备份与载荷"></a>
#### 备份与载荷
...
<a id="deploy-records-qrd-002-deployment-md-s3-迁移与回滚用法"></a>

<a id="deploy-records-qrd-002-deployment-md-迁移与回滚用法"></a>
#### 迁移与回滚用法

迁移实现见 [migrate_workspace.sh](../../../devices/go2/profiles/go2_robot2/migrate_workspace.sh)，日常启动和原生参数
对应关系见 [DEPLOYMENT.md](DEPLOYMENT_GUIDE.md#deploy-go2-robot2-deployment-md)。以下 `--check/--apply` 仅适用于
...
<a id="deploy-records-qrd-002-deployment-md-source-4"></a>

<a id="deploy-records-qrd-002-deployment-md-历史材料-4deployedge_device_deployment_requestmd"></a>
## 历史材料 4：deploy/EDGE_DEVICE_DEPLOYMENT_REQUEST.md

> 归档原文；以下命令、状态与结论按原日期理解。

<a id="deploy-records-qrd-002-deployment-md-s4-ccs-端侧部署总结与可复用任务指令"></a>

<a id="deploy-records-qrd-002-deployment-md-ccs-端侧部署总结与可复用任务指令"></a>
### CCS 端侧部署总结与可复用任务指令
...
<a id="deploy-records-qrd-002-deployment-md-s4-一本次-go2-部署总结"></a>

<a id="deploy-records-qrd-002-deployment-md-一本次-go2-部署总结"></a>
#### 一、本次 Go2 部署总结
...
<a id="deploy-records-qrd-002-deployment-md-s4-二可直接发送的部署指令"></a>

<a id="deploy-records-qrd-002-deployment-md-二可直接发送的部署指令"></a>
#### 二、可直接发送的部署指令

请依据以下参数，在指定目标设备上完成 CCS 端侧功能部署或迁移。读取代码和实机后直接实施，完成本地源码同步、必要构建、增量验证、部署记录，以及参数区指定的 Git 交付。不要只停留在方案说明。

<a id="deploy-records-qrd-002-deployment-md-s4-1-目标参数与执行范围"></a>

<a id="deploy-records-qrd-002-deployment-md-1-目标参数与执行范围"></a>
##### 1. 目标参数与执行范围
...
<a id="deploy-records-qrd-002-deployment-md-s4-2-完成目标与适配原则"></a>

<a id="deploy-records-qrd-002-deployment-md-2-完成目标与适配原则"></a>
##### 2. 完成目标与适配原则
...
<a id="deploy-records-qrd-002-deployment-md-s4-3-阶段-a只读盘点与确定接口"></a>

<a id="deploy-records-qrd-002-deployment-md-3-阶段-a只读盘点与确定接口"></a>
##### 3. 阶段 A：只读盘点与确定接口
...
<a id="deploy-records-qrd-002-deployment-md-s4-4-阶段-b本地实现与独立配置"></a>

<a id="deploy-records-qrd-002-deployment-md-4-阶段-b本地实现与独立配置"></a>
##### 4. 阶段 B：本地实现与独立配置
...
<a id="deploy-records-qrd-002-deployment-md-s4-5-阶段-c通信建图和任务契约"></a>

<a id="deploy-records-qrd-002-deployment-md-5-阶段-c通信建图和任务契约"></a>
##### 5. 阶段 C：通信、建图和任务契约
...
<a id="deploy-records-qrd-002-deployment-md-s4-6-阶段-d备份迁移与必要构建"></a>

<a id="deploy-records-qrd-002-deployment-md-6-阶段-d备份迁移与必要构建"></a>
##### 6. 阶段 D：备份、迁移与必要构建
...
<a id="deploy-records-qrd-002-deployment-md-s4-7-阶段-e增量验收与统一切换"></a>

<a id="deploy-records-qrd-002-deployment-md-7-阶段-e增量验收与统一切换"></a>
##### 7. 阶段 E：增量验收与统一切换
...
<a id="deploy-records-qrd-002-deployment-md-s4-8-阶段-f文档发布与最终交付"></a>

<a id="deploy-records-qrd-002-deployment-md-8-阶段-f文档发布与最终交付"></a>
##### 8. 阶段 F：文档、发布与最终交付
...
<a id="deploy-records-qrd-002-deployment-md-后续记录填写格式"></a>
## 后续记录填写格式

追加日期/范围、源码版本与差异、文件清单及前后哈希、备份/证据路径、命令与实测、告警/未测项、启停/回滚。回滚不覆盖更新的急停状态，文档整理不表示重新部署。

<a id="deploy-records-qrd-002-deployment-md-camera-emergency-20260911"></a>

<a id="deploy-records-qrd-002-deployment-md-2026-09-11同步-go2_3-相机启动与急停修复"></a>
## 2026-09-11：同步 GO2_3 相机启动与急停修复

<a id="deploy-records-qrd-002-deployment-md-范围与实测原因"></a>
### 范围与实测原因
...
<a id="deploy-records-qrd-002-deployment-md-实现"></a>
### 实现
...
<a id="deploy-records-qrd-002-deployment-md-备份与运行文件哈希"></a>
### 备份与运行文件哈希
...
<a id="deploy-records-qrd-002-deployment-md-增量验证及限制"></a>
### 增量验证及限制
...
<a id="deploy-records-qrd-002-deployment-md-下次切换与恢复"></a>
### 下次切换与恢复
...
<a id="deploy-records-qrd-002-deployment-md-joint-shutdown-20260912"></a>
<a id="deploy-records-qrd-002-deployment-md-2026-09-12-联合下发拒绝与退出急停修复已部署"></a>
## 2026-09-12 联合下发拒绝与退出急停修复（已部署）

<a id="deploy-records-qrd-002-deployment-md-根因与变更边界"></a>
### 根因与变更边界
...
<a id="deploy-records-qrd-002-deployment-md-备份与源码身份"></a>
### 备份与源码身份
...
<a id="deploy-records-qrd-002-deployment-md-增量验收实测"></a>
### 增量验收实测
...
<a id="deploy-records-qrd-002-deployment-md-最终运行日志与回滚"></a>
### 最终运行、日志与回滚
...
<a id="deploy-records-qrd-002-deployment-md-field-confirmation-20260912"></a>
<a id="deploy-records-qrd-002-deployment-md-2026-09-12-后续现场确认与经验归档"></a>
...
复用规则已归档到 [GO2 部署经验](DEPLOYMENT_GUIDE.md#documents-go2-deployment-lessons-md)，并同步更新从零部署指南、接口参考、使用手册、任务包 README 与部署请求模板：先按 recorded_at/reason 查锁存，统一幂等收尾并排除未完成 RPC，区分卸载复用和永久关闭，在传输时限内连续完成联合下发并检查本轮 XML 落盘，按证据来源记录现场确认。

本次是本地文档整理，没有重新连接、启停或改变设备运行参数。端侧历史文档及清单保持；下一次交付按部署指南的离线目录层级同步本次文档，再生成新的文档清单，不覆盖旧验收证据。

<a id="deploy-records-qrd-003-deployment-md"></a>

<a id="deploy-records-qrd-003-deployment-md-qrd_003-部署与验收记录"></a>
# QRD_003 部署与验收记录

合并日期：2026-09-11；操作基线：CCS 0.23.1。此文件是该设备唯一的部署记录入口，后续按日期追加。

<a id="deploy-records-qrd-003-deployment-md-当前入口"></a>
## 当前入口

- 设备：QRD_003；profile：`go2_robot3`；端侧：`unitree@192.168.50.112`。
- [配置与脚本](../../../devices/go2/profiles/go2_robot3)保持原位置；[从零部署](../../USER_MANUAL.md#documents-deployment-guide-md)、[接口填写](../../INTERFACE_REFERENCE.md#documents-config-topic-reference-md)、[使用手册](../../USER_MANUAL.md#documents-user-manual-md)、[设备索引](../../../README.md)。
- 原生 Go2：相机自动选择/RGB freshness、SNTP 可用性检查、独立进程会话、有序停用、持久急停和按启动时间日志。2026-09-09 相机未连接/跳过记录保留，9 月 10 日有后续实测。2026-09-12 已部署适配器退出急停修复（含 shutdown_requested）、完成服务复位与重启，联合下发通过，最终 disabled、定位 standby；见[最新记录](DEPLOYMENT_RECORD.md#deploy-records-qrd-003-deployment-md-joint-shutdown-20260912)。其他存储/根脚本改动不能视为随本轮部署。

- 2026-09-12 后续：用户已确认本轮落地测试完成，见[现场确认](DEPLOYMENT_RECORD.md#deploy-records-qrd-003-deployment-md-field-confirmation-20260912)。此前 disabled/standby 是工具验收时快照，不作为当前实时状态；后续未部署修订的状态保持。

<a id="deploy-records-qrd-003-deployment-md-历史材料与来源校验"></a>
## 历史材料与来源校验
...
| 原文件（相对 edge_side_pkg） | 原始字节 SHA-256 | 合并章节 |
| --- | --- | --- |
| `deploy/go2_robot3/README.md` | `a350bc1f8dee22a1eba176dc175c1188d39eb6c6614f57278d72913f36e68bdf` | [材料 1](DEPLOYMENT_RECORD.md#deploy-records-qrd-003-deployment-md-source-1) |
| `deploy/go2_robot3/DEPLOYMENT.md` | `a59611c15aba8b2852b4fdd70b68b075e3481344622f0bb08a191c935ec0202d` | [材料 2](DEPLOYMENT_RECORD.md#deploy-records-qrd-003-deployment-md-source-2) |
| `deploy/go2_robot3/VALIDATION.md` | `13c16f8fbbe94c821310443a5953aac9e46a0d6d0d008835549fb51db0a1019d` | [材料 3](DEPLOYMENT_RECORD.md#deploy-records-qrd-003-deployment-md-source-3) |

<a id="deploy-records-qrd-003-deployment-md-source-1"></a>

<a id="deploy-records-qrd-003-deployment-md-历史材料-1deploygo2_robot3readmemd"></a>
## 历史材料 1：deploy/go2_robot3/README.md

> 归档原文；以下命令、状态与结论按原日期理解。

<a id="deploy-records-qrd-003-deployment-md-s1-go2-robot-3-qrd_003"></a>

<a id="deploy-records-qrd-003-deployment-md-go2-robot-3-qrd_003"></a>
### GO2 Robot 3: QRD_003
...
<a id="deploy-records-qrd-003-deployment-md-s1-daily-operation"></a>

<a id="deploy-records-qrd-003-deployment-md-daily-operation"></a>
#### Daily Operation
...
<a id="deploy-records-qrd-003-deployment-md-s1-runtime-monitoring"></a>

<a id="deploy-records-qrd-003-deployment-md-runtime-monitoring"></a>
#### Runtime Monitoring
...
<a id="deploy-records-qrd-003-deployment-md-s1-runtime-layout"></a>

<a id="deploy-records-qrd-003-deployment-md-runtime-layout"></a>
#### Runtime Layout
...
<a id="deploy-records-qrd-003-deployment-md-s1-hardware-and-services"></a>

<a id="deploy-records-qrd-003-deployment-md-hardware-and-services"></a>
...
<a id="deploy-records-qrd-003-deployment-md-source-2"></a>

<a id="deploy-records-qrd-003-deployment-md-历史材料-2deploygo2_robot3deploymentmd"></a>
## 历史材料 2：deploy/go2_robot3/DEPLOYMENT.md

> 归档原文；以下命令、状态与结论按原日期理解。

<a id="deploy-records-qrd-003-deployment-md-s2-qrd_003-deployment-procedure"></a>

<a id="deploy-records-qrd-003-deployment-md-qrd_003-deployment-procedure"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s2-installation"></a>

<a id="deploy-records-qrd-003-deployment-md-installation"></a>
#### Installation
...
<a id="deploy-records-qrd-003-deployment-md-s2-acceptance-evidence"></a>

<a id="deploy-records-qrd-003-deployment-md-acceptance-evidence"></a>
#### Acceptance Evidence
...
<a id="deploy-records-qrd-003-deployment-md-s2-mapping-script-line-ending-increment"></a>

<a id="deploy-records-qrd-003-deployment-md-mapping-script-line-ending-increment"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s2-runtime-monitor-increment"></a>

<a id="deploy-records-qrd-003-deployment-md-runtime-monitor-increment"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s2-rollback"></a>

<a id="deploy-records-qrd-003-deployment-md-rollback"></a>
#### Rollback
...
<a id="deploy-records-qrd-003-deployment-md-s2-emergency-latch-and-per-start-log-increment"></a>

<a id="deploy-records-qrd-003-deployment-md-emergency-latch-and-per-start-log-increment"></a>
...
<a id="deploy-records-qrd-003-deployment-md-source-3"></a>

<a id="deploy-records-qrd-003-deployment-md-历史材料-3deploygo2_robot3validationmd"></a>
## 历史材料 3：deploy/go2_robot3/VALIDATION.md

> 归档原文；以下命令、状态与结论按原日期理解。

<a id="deploy-records-qrd-003-deployment-md-s3-qrd_003-validation-record"></a>

<a id="deploy-records-qrd-003-deployment-md-qrd_003-validation-record"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-result-summary"></a>

<a id="deploy-records-qrd-003-deployment-md-result-summary"></a>
#### Result summary
...
<a id="deploy-records-qrd-003-deployment-md-s3-startup-script-incremental-update-2026-09-10"></a>

<a id="deploy-records-qrd-003-deployment-md-startup-script-incremental-update-2026-09-10"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-mapping-negotiation-line-ending-repair-2026-09-10"></a>

<a id="deploy-records-qrd-003-deployment-md-mapping-negotiation-line-ending-repair-2026-09-10"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-runtime-first-map-export-repair-2026-09-10-1558"></a>

<a id="deploy-records-qrd-003-deployment-md-runtime-first-map-export-repair-2026-09-10-1558"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-source-payload-and-backup"></a>

<a id="deploy-records-qrd-003-deployment-md-source-payload-and-backup"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-build-and-static-validation"></a>

<a id="deploy-records-qrd-003-deployment-md-build-and-static-validation"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-target-preflight-and-contracts"></a>

<a id="deploy-records-qrd-003-deployment-md-target-preflight-and-contracts"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-live-sensor-sample"></a>

<a id="deploy-records-qrd-003-deployment-md-live-sensor-sample"></a>
#### Live sensor sample
...
<a id="deploy-records-qrd-003-deployment-md-s3-udp-capture"></a>

<a id="deploy-records-qrd-003-deployment-md-udp-capture"></a>
#### UDP capture
...
<a id="deploy-records-qrd-003-deployment-md-s3-platform-reload-and-mqtt-capture"></a>

<a id="deploy-records-qrd-003-deployment-md-platform-reload-and-mqtt-capture"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-camera-and-one-click-startup"></a>

<a id="deploy-records-qrd-003-deployment-md-camera-and-one-click-startup"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-safe-shutdown-and-remaining-acceptance"></a>

<a id="deploy-records-qrd-003-deployment-md-safe-shutdown-and-remaining-acceptance"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-startup-time-service-availability-update-2026-09-10"></a>

<a id="deploy-records-qrd-003-deployment-md-startup-time-service-availability-update-2026-09-10"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-runtime-watchdog-correction-2026-09-10"></a>

<a id="deploy-records-qrd-003-deployment-md-runtime-watchdog-correction-2026-09-10"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-emergency-stop-recovery-and-per-start-logs-2026-09-10"></a>

<a id="deploy-records-qrd-003-deployment-md-emergency-stop-recovery-and-per-start-logs-2026-09-10"></a>
...
<a id="deploy-records-qrd-003-deployment-md-s3-local-pre-pr-review-follow-up-2026-09-11"></a>

<a id="deploy-records-qrd-003-deployment-md-local-pre-pr-review-follow-up-2026-09-11"></a>
...
<a id="deploy-records-qrd-003-deployment-md-后续记录填写格式"></a>
## 后续记录填写格式

追加日期/范围、源码版本与差异、文件清单及前后哈希、备份/证据路径、命令与实测、告警/未测项、启停/回滚。回滚不覆盖更新的急停状态，文档整理不表示重新部署。


<a id="deploy-records-qrd-003-deployment-md-joint-shutdown-20260912"></a>
<a id="deploy-records-qrd-003-deployment-md-2026-09-12-联合下发拒绝与退出急停修复已部署"></a>
## 2026-09-12 联合下发拒绝与退出急停修复（已部署）

<a id="deploy-records-qrd-003-deployment-md-根因与变更边界"></a>
### 根因与变更边界
...
<a id="deploy-records-qrd-003-deployment-md-备份与源码身份"></a>
### 备份与源码身份
...
<a id="deploy-records-qrd-003-deployment-md-增量验收实测"></a>
### 增量验收实测
...
<a id="deploy-records-qrd-003-deployment-md-最终运行日志与回滚"></a>
### 最终运行、日志与回滚
...
<a id="deploy-records-qrd-003-deployment-md-field-confirmation-20260912"></a>
<a id="deploy-records-qrd-003-deployment-md-2026-09-12-后续现场确认与经验归档"></a>
## 2026-09-12 后续现场确认与经验归档

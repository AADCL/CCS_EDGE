...
历史before.tar.gz、before-camera-fix-1438.tar.gz是故障回滚旧状态，不是最终模板。端侧测试完成后的清理已将历史备份、验收脚本和旧会话日志转存至部署电脑 `CCS_dev/artifacts/ugv004_fix_20260924/edge_cleanup_20260924.tar.gz`（SHA-256：`aabd71ea6ecae235c69ec0a2733db9e7d34c213ac444136955d9d322c6228d55e`），端侧不再保留这些旧文件；当前 `logs/latest` 和可复用的 `backups/ugv004-template-20260924` 保留。未来一键安装固定为：校验归档→核对镜像/契约→拒绝覆盖→替换参数/记录指纹→CCS构建→预检→明确启动/验收。分别报告安装、启动、定位和导航ready，不吞错、不同步时不启动、不自动修改原生代码、不自动发运动任务。

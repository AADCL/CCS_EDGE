...
UAV 使用独立八包工作空间和与 UGV_004 相同的根目录启动形式；无参数为 mapping 模式，飞行权限仍须显式开启。详见 [UAV 部署与启动指南](devices/uav/DEPLOYMENT_GUIDE.md) 和 [部署记录](devices/uav/DEPLOYMENT_RECORD.md)。本轮未进行实飞。
